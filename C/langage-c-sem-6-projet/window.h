...
/**
 * Modifie la couleur d'arrière-plan de la fenêtre, qui est appliquée comme base à chaque
 * repaint (par défaut cette couleur est le noir, a priori).
 *
...
/**
 * Charge une police de caractères dans la base interne de polices de caractère, avec
 * le nom donné et la taille donné (deux polices deux tailles différentes doivent chacun
 * être chargées séparément).
 *
 * En cas d'échec (si la police n'est pas accessible par exemple), renvoie -1. Sinon, la
 * police renvoie un identifiant (un font_id_t) à utiliser pour les autres fonctions du
 * module qui manipulent du texte.
 *
 ...
 * (Note: en réalité, la fonction est robuste sur ces derniers points ; si la fenêtre, la surface ou le
...
/**
 * Modifie la couleur de rendu, utilisée pour afficher des formes. L'utilisateur ne devrait probablement
 * pas appeler cette fonction directement, comme les couleurs sont gérées par chaque fonction.
 *
 ...
 * Les coordonnées peuvent être en dehors de la fenêtre a priori (auquel cas des points invisibles
 * sont dessinés).
 *
...
/**
 * Rend une ligne dans la fenêtre (non-aliasée) entre les points donnés et de couleur donnée,
 * avec l'épaisseur donnée, et dont la cible se termine avec une tête de flèche (un V).
 *
 * Pré-conditions : 
 *   - window non NULL, window lancée, thread == render thread de la window
 *   - size > 0.0
 ...
 * @param alpha angle de la tête de flèche (angle entre la ligne et le battant d'un côté de la flèche)
 * @param thickness épaisseur du trait
 * @param color couleur de la ligne
 */
...
/**
 * Rend une ligne courbe dans la fenêtre (non-aliasée) entre les points donnés et de couleur donnée,
 * et dont la cible se termine avec une tête de flèche (un V). La courbure est obtenue avec des 
 * points de contrôle, calculés à partir du paramètre "curving".
 *
 ...
 * sont dessinés).
 *
 * "curving" représente la distance entre l'axe (x1,y1)-(x2,y2) et les points de contrôle. Elle peut
 * être positive, auquel cas la ligne est courbée vers l'extérieur, ou négative auquel cas elle est
 * courbée vers l'intérieur. Plus formellement, "l'extérieur" est dans le sens du vecteur normal à
 * l'axe (donc le vecteur orthogonal direct au vecteur (x2 - x1, y2 - y1)).
 *
 * @param window fenêtre dans laquelle dessiner la ligne
 * @param x1 abscisse du point d'origine
 * @param y1 ordonnée du point d'origine
 ...
 *
 * Pré-conditions : window non NULL, window lancée, thread = render thread de la fenêtre
 * rect != NULL
 *
...
/**
 * Rend du texte sur la fenêtre, avec la police chargée spécifiée, à la position spécifiée et l'alignement
 * donné, et avec la couleur d'avant et d'arrière plan données.
 *
 * Pré-conditions : window non NULL, window lancée, thread = render thread de la fenêtre
 * La valeur d'alignement doit être un OU d'une valeur VALIGN et d'une valeur HALIGN.
 *
 * @param window fenêtre sur laquelle opérer le rendu
 * @param text texte à rendre
 * @param fontid identifiant de police interne à la fenêtre (obtenu avec @ref load_font)
