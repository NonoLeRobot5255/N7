...
switch fonction
    case 0 %eig
        [Vp,diag_vp]=eig(M);
    case 1 %powerv12
         [ Vp, diag_vp, ~, ~, ~ ] = power_v12( M, search_space, percentage, eps, maxit );
    case 2 %subspaceiterv0
        [Vp,diag_vp,~,~]= subspace_iter_v0(M, search_space, eps, maxit);
    case 3 %subspaceiterv1
        [Vp,diag_vp, ~, ~, ~, ~ ] = subspace_iter_v1( M, search_space, percentage, eps, maxit );
    case 4 %subspaceiterv2
        [Vp,diag_vp, ~, ~, ~, ~ ] = subspace_iter_v2( M, search_space, percentage, puiss, eps, maxit );
    case 5 %subspaceiterv3
        [Vp,diag_vp, ~, ~, ~, ~ ] = subspace_iter_v3( M, search_space, percentage, puiss, eps, maxit );
end
...
% calcul des meilleures approximations de rang faibledifferenceSVD = zeros(size(inter,2), 1);
differenceSVD_postcompr = zeros(size(inter,2), 1);
% images reconstruites en utilisant de 1 à 200 vecteurs (avec un pas de 40)
td = 0;
if(tourner)
    I=I';
...
figure(ti)
hold on 
plot(inter, differenceSVD_postcompr, 'rx')
ylabel('RMSE')
xlabel('rank k')
%%
